...
*  Program: otp_dec.c
*  Date: 3/17/2018
*  Description: One-time-tab client side, sends key and decoded files
*				for server side decoding
************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void sendDec(int fileD[2], int sockFD){
	...
	// Check that the file is free from bad characters
	for(i = 0; i < k; i++){
		for(j = 0; j < 27; j++){
...
/***********************************************************************
*  Function: 		recvDecMsg()
*  Description:  	Receives and prints decoded message from server
...
void recvDecMsg(int sockFD, int fSize){
	int i;
	// buffer to hold decoded message from server
	char dataBuffer[MAX_BUFFER];
	// size of message
	...
	// Receive decoded message and store in buffer
	while(totalBytes > 0){
		int k = recv(sockFD, dataBuffer, sizeof(dataBuffer), 0);
		if(k < 0){
			fprintf(stderr, "ERROR: Error receiving decoded message");
			exit(1);
		}
		totalBytes -= k;
	}

	// Print decoded message in buffer
	for(i = 0; i < fSize; i++){
...
/***********************************************************************
*  Function: 		sendSizes()
*  Description:  	Sends size of file to decoding server
*  Parameters:   	file and socket descriptors
*  Pre-conditions:	socket is connected
*  Post-conditions:	None
*  Return:		size of file
...
int main(int argc, char *argv[])
{
	...
	// Send id to verify self as decoding client
	verifyID(socketFD);
